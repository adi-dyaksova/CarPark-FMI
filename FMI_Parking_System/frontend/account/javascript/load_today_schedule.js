    ...
    for (let i = 0; i < courses.length; i++) {
      ...
      data.forEach((row) => {
        if (row["discipline_name"] === courses[i]) {
          ...
          li.innerHTML += "<strong>" + row["discipline_type"] + ":</strong>" + " " + row["start_time"] + " - " + row["end_time"] + " (" + row["faculty"] + ")";
          ul.appendChild(li);
        }
      })

      /*         Пример:
        Лекция: 10:00 - 12:00 (ФМИ)
        Лекция: 18:00 - 20:00 (ФМИ)
        Упражнение: 18:00 - 20:00 (ФМИ)
      */
